...
def length_angle_inft(dw, min_m, max_m, min_t, max_t, size=20):
    ...
    for j, m2 in enumerate(mm2):
        for i, m1 in enumerate(mm1):
            matrix1[j, i] = conc_length_mismatch(settings, m1, m2)
    ...
    for j, t2 in enumerate(tt2):
        for i, t1 in enumerate(tt1):
            matrix2[j, i] = conc_angle_mismatch(settings, t1, t2)

    xtick_labels1 = np.linspace(min(mm1), max(mm1), num=5)  
    ytick_labels1 = np.linspace(min(mm2), max(mm2), num=5)
    xlabel1 = '\\textit{$\\alpha_1$} (m)'
    ylabel1 = '\\textit{$\\alpha_2$} (m)'
    xtick_labels2 = np.linspace(min(tt1), max(tt1), num=5)  
    ytick_labels2 = np.linspace(min(tt2), max(tt2), num=5)
    xlabel2 = '\\textit{$t_1$} (deg)'
    ylabel2 = '\\textit{$t_2$} (deg)'

    return plot_length_angle_mismatch(f'Infinitesimal Bin {dw/1e12}THz Detuning',
                               matrix1=matrix1, 
                               mx1_xlabel=xlabel1, 
...
def length_angle_st(bw, min_m, max_m, min_t, max_t, N=100, size=20):
    ...
    return plot_length_angle_mismatch(f' {bw/1e12}THz Bandwidth ',
                               matrix1=matrix1, 
                               mx1_xlabel=xlabel1, 
                               mx1_xticks=xtick_labels1, 
                               mx1_ylabel=ylabel1,
...
def length_angle_st_slices(bws, min_m, max_m, min_t, max_t, N=100, size=20):
    settings = {
        'lambda_deg':   1556e-9,
        'l1':           1.000,
        'l1_p':         1.000,
    ...
    # Generate the matrix using the concurrence function
    mm = np.linspace(min_m,max_m,size)
    matrix1 = np.zeros((len(bws), size))
    for j, m in enumerate(mm):
        for i, bw in enumerate(bws):
            settings['filter_range'] = np.linspace(0.0,0.5*bw,N)
            matrix1[i, j] = conc_length_mismatch(settings, m, m)
    ...
    # Generate the matrix using the concurrence function
    tt = np.linspace(min_t,max_t,size)
    matrix2 = np.zeros((len(bws), size))
    for j, t in enumerate(tt):
        for i, bw in enumerate(bws):
            settings['filter_range'] = np.linspace(0.0,0.5*bw,N)
            matrix2[i, j] = conc_angle_mismatch(settings, t, t)

    bws = bws / 1e12
    xtick_labels1 = np.linspace(min(mm), max(mm), num=5)  
    ytick_labels1 = np.linspace(min(bws), max(bws), num=5)
    xlabel1 = '\\textit{$\\alpha$} (m)'
    ylabel1 = '\\textit{$bw$} (THz)'
    xtick_labels2 = np.linspace(min(tt), max(tt), num=5)  
    ytick_labels2 = np.linspace(min(bws), max(bws), num=5)
    xlabel2 = '\\textit{$t$} (deg)'
...
def length_angle_dt(bw, dw, min_m, max_m, min_t, max_t, N=100, size=20):
    ...
    return plot_length_angle_mismatch(f'{dw/1e12}THz Detuned, {bw/1e12}THz Bandwidth ',
                               matrix1=matrix1, 
                               mx1_xlabel=xlabel1, 
                               mx1_xticks=xtick_labels1, 
                               mx1_ylabel=ylabel1,
...
def length_angle_dt_slices(bw, dws, min_m, max_m, min_t, max_t, N=100, size=20):
    ...
    for j, m in enumerate(mm):
        for i, dw in enumerate(dws):
            settings['filter_range'] = np.linspace(dw-0.5*bw,dw+0.5*bw,N)
            matrix1[i, j] = conc_length_mismatch(settings, m, m)
    ...
    print(np.min(matrix1), np.max(matrix1))
    ...
    plot_length_angle_mismatch(f' Varying Detuning of {bw/1e12}THz Dual-tooth Filter',
                               matrix1=matrix1, 
                               mx1_xlabel=xlabel1, 
                               mx1_xticks=xtick_labels1, 
                               mx1_ylabel=ylabel1,
